...
import { Leaf, Recycle, Trophy, MapPin } from "lucide-react";
...
async function getUserData(email: string) {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: {
      ...
      email: true,
      image: true,
      points: true,        // Ensure these exist in prisma.schema
      recycledCount: true, 
      level: true
    }
  });
...
export default async function Home() {
  ...
  // If user is null (database error), handle it gracefully
  if (!user) return <div>User not found</div>;

  const stats = {
    points: user.points ?? 0,          // Use '?? 0' to handle nulls
    recycled: user.recycledCount ?? 0,
    level: user.level ?? 1,
  };

  return (
    <main className="min-h-screen bg-green-50 dark:bg-slate-900 transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Ready to make a difference today?</p>
        </div>

        {/* STATS GRID */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {/* Use this pattern for all cards: bg-white dark:bg-slate-800 border-green-100 dark:border-slate-700 */}
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-green-100 dark:border-slate-700 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800 dark:text-white">{stats.recycled}</p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-green-100 dark:border-slate-700 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800 dark:text-white">{(stats.recycled * 0.15).toFixed(1)}kg</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button className="bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-slate-700 p-4 rounded-xl flex flex-col items-center justify-center gap-2 transition">
            <Recycle className="w-6 h-6" />
            <span className="font-bold">Scan Item</span>
          </button>
        </div>
